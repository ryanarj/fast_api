...
class PydanticModelField:
    ...
    def to_argument(
        self, current_info: TypeInfo, typed: bool, force_optional: bool, use_alias: bool
    ) -> Argument:
...
class PydanticModelTransformer:
    ...
    def transform(self) -> bool:
        ...
        * stores the fields, config, and if the class is settings in the mypy metadata for access by subclasses
        ...
        info = self._cls.info
        is_root_model = any(
            ROOT_MODEL_FULLNAME in base.fullname for base in info.mro[:-1]
        )
        ...
        is_settings = any(
            base.fullname == BASESETTINGS_FULLNAME for base in info.mro[:-1]
        )
    ...
    def add_initializer(
        ...
        requires_dynamic_aliases = bool(
            config.has_alias_generator and not config.populate_by_name
        )
        with state.strict_optional_set(self._api.options.strict_optional):
                ...
                if "__init__" in base_settings_node.names:
                    ...
                    if (
                        ...
                        for arg_idx, arg_name in enumerate(func_type.arg_names):
                            if arg_name.startswith("__") or not arg_name.startswith(
                                "_"
                            ):
                                continue
                            ...
                            args.append(
                                Argument(
                                    variable, analyzed_variable_type, None, ARG_OPT
                                )
                            )
