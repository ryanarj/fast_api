...
GetCoreSchemaFunction = Callable[
...
def check_decorator_fields_exist(
    ...
    for dec in decorators:
        if (
            isinstance(dec.info, (ValidatorDecoratorInfo, FieldValidatorDecoratorInfo))
            and "*" in dec.info.fields
        ):
            continue
...
class GenerateSchema:
    ...
    def _apply_discriminator_to_union(
        ...
        try:
            ...
            if "metadata" in schema:
                ...
                schema["metadata"] = {
                    NEEDS_APPLY_DISCRIMINATED_UNION_METADATA_KEY: True
                }
    ...
    def _model_schema(self, cls: type[BaseModel]) -> core_schema.CoreSchema:
        ...
        with self.defs.get_schema_or_ref(cls) as (model_ref, maybe_schema):
            ...
            metadata = build_metadata_dict(
                js_functions=[partial(modify_model_json_schema, cls=cls)]
            )
            ...
            if core_config.get("extra_fields_behavior") == "allow":
                for tp in (cls, *cls.__mro__):
                    extras_annotation = cls.__annotations__.get(
            ...
            with self._config_wrapper_stack.push(config_wrapper):
                ...
                if cls.__pydantic_root_model__:
                    ...
                    new_inner_schema = define_expected_missing_refs(
                ...
                self.defs.definitions[model_ref] = self._post_process_generated_schema(
                    schema
                )
    ...
    def _generate_schema_from_property(
        ...
        if get_schema is None:
            ...
            if len(inspect.signature(get_schema).parameters) == 1:
                ...
                schema = get_schema(
                    source,
                    CallbackGetCoreSchemaHandler(
                        self._generate_schema, self, ref_mode=ref_mode
                    ),
                )
    ...
    def _generate_schema(self, obj: Any) -> core_schema.CoreSchema:
        ...
        self._needs_apply_discriminated_union = (
            self._needs_apply_discriminated_union or needs_apply_discriminated_union
        )
        return schema
    ...
    def _common_field_schema(  # noqa C901
        ...
        if has_instance_in_type(field_info.annotation, (ForwardRef, str)):
            ...
            evaluated = _typing_extra.eval_type_lenient(
                field_info.annotation, types_namespace, None
            )
            if evaluated is not field_info.annotation and not has_instance_in_type(
                evaluated, PydanticRecursiveRef
            ):
        ...
        with self.field_name_stack.push(name):
            if field_info.discriminator is not None:
                schema = self._apply_annotations(
                    source_type, annotations, transform_inner_schema=set_discriminator
                )
            else:
                schema = self._apply_annotations(
                    source_type,
        ...
        metadata = build_metadata_dict(
            js_annotation_functions=[
                get_json_schema_update_func(json_schema_updates, json_schema_extra)
            ]
        )
    ...
    def _type_alias_type_schema(
        ...
        with self.defs.get_schema_or_ref(obj) as (ref, maybe_schema):
            ...
            new_namespace = {
                **_typing_extra.get_cls_types_namespace(origin),
                **namespace,
            }
            ...
            annotation = _typing_extra.eval_type_lenient(
                annotation, self._types_namespace, None
            )
    ...
    def _typed_dict_schema(
        ...
        with self.defs.get_schema_or_ref(typed_dict_cls) as (
            typed_dict_ref,
            maybe_schema,
        ):
            if maybe_schema is not None:
            ...
            with self._config_wrapper_stack.push(config):
                ...
                metadata = build_metadata_dict(
                    js_functions=[
                        partial(modify_model_json_schema, cls=typed_dict_cls)
                    ],
                ...
                self.defs.definitions[
                    typed_dict_ref
                ] = self._post_process_generated_schema(schema)
    ...
    def _namedtuple_schema(
        ...
        with self.defs.get_schema_or_ref(namedtuple_cls) as (
            namedtuple_ref,
            maybe_schema,
        ):
            if maybe_schema is not None:
    ...
    def _subclass_schema(self, type_: Any) -> core_schema.CoreSchema:
        ...
        if type_param == Any:
            ...
            if type_param.__bound__:
                ...
                return core_schema.union_schema(
                    [
                        self.generate_schema(typing.Type[c])
                        for c in type_param.__constraints__
                    ]
                )
            else:
                return self._type_schema()
    ...
    def _sequence_schema(self, sequence_type: Any) -> core_schema.CoreSchema:
        ...
        return core_schema.json_or_python_schema(
            json_schema=list_schema, python_schema=python_schema
        )
    ...
    def _computed_field_schema(
        ...
        try:
            return_type = _decorators.get_function_return_type(
                d.func, d.info.return_type, self._types_namespace
            )
        except NameError as e:
            raise PydanticUndefinedAnnotation.from_name_error(e) from e
        ...
        alias_generator = self._config_wrapper.alias_generator
        if alias_generator and (
            d.info.alias_priority is None or d.info.alias_priority <= 1
        ):
        ...
        def set_computed_field_metadata(
        ...
        metadata = build_metadata_dict(
            js_annotation_functions=[set_computed_field_metadata]
        )
    ...
    def _apply_annotations(
        ...
        res = self._get_prepare_pydantic_annotations_for_known_type(
            source_type, tuple(annotations)
        )
        if res is not None:
            source_type, annotations = res
        ...
        if pydantic_js_annotation_functions:
            ...
            metadata.setdefault("pydantic_js_annotation_functions", []).extend(
    ...
    def _apply_single_annotation_json_schema(
        ...
        if isinstance(metadata, FieldInfo):
            ...
            if json_schema_update or json_schema_extra:
                CoreMetadataHandler(schema).metadata.setdefault(
                    "pydantic_js_annotation_functions", []
                ).append(
    ...
    def _get_wrapped_inner_schema(
        ...
        metadata_get_schema: GetCoreSchemaFunction = getattr(
            annotation, "__get_pydantic_core_schema__", None
        ) or (lambda source, handler: handler(source))
...
def _extract_get_pydantic_json_schema(
    tp: Any, schema: CoreSchema
    ...
    if hasattr(tp, "__origin__") and not isinstance(
        tp, type(Annotated[int, "placeholder"])
    ):
...
class _Definitions:
    ...
    def get_schema_or_ref(
        self, tp: Any
    ) -> Iterator[tuple[str, None] | tuple[str, CoreSchema]]:
...
def generate_pydantic_signature(
    ...
    if var_kw and use_var_kw:
        ...
        while var_kw_name in fields:
            var_kw_name += "_"
        merged_params[var_kw_name] = post_process_parameter(
            var_kw.replace(name=var_kw_name)
        )
