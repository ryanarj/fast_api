...
from collections.abc import (
    Callable,
    Hashable as CollectionsHashable,
    Iterable as CollectionsIterable,
)
from typing import (
    TYPE_CHECKING,
    Any,
        ...
        for attr_name, value in from_config.items():
            try:
                ...
                if current_value is self.__field_constraints__.get(attr_name, None):
                    ...
                    self.include = ValueItems.merge(
                        value, current_value, intersect=True
                    )
...
class ModelField(Representation):
    ...
    def __init__(
        ...
        *,
        ...
        self.sub_fields_mapping: Optional[
            Dict[str, "ModelField"]
    ...
    def set_config(self, config: Type["BaseConfig"]) -> None:
        ...
        if new_exclude is not None:
            self.field_info.exclude = ValueItems.merge(
                self.field_info.exclude, new_exclude
            )
    ...
    def _set_default_and_type(self) -> None:
        ...
        if self.type_ is Undefined:
            raise errors_.ConfigError(
                f'unable to infer type for attribute "{self.name}"'
            )
    ...
    def validate(
        ...
        *,
        ...
        if v is None:
            if is_none_type(self.type_):
                ...
                if self.post_validators:
                    return self._apply_validators(
                        v, values, loc, cls, self.post_validators
                    )
                else:
                    return None, None
            else:
        ...
        if not errors and self.post_validators:
            v, errors = self._apply_validators(
                v, values, loc, cls, self.post_validators
            )
        return v, errors
    ...
    def _validate_mapping_like(
        ...
        for k, v_ in v_iter.items():
            ...
            v_loc = *loc, k
            value_result, value_errors = self._validate_singleton(
                v_, values, v_loc, cls
            )
            if value_errors:
...
def is_finalvar_with_default_val(type_: Type[Any], val: Any) -> bool:
    return (
        is_finalvar(type_) and val is not Undefined and not isinstance(val, FieldInfo)
    )
